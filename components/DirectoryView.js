...
import { Card, Title, Paragraph, Button, IconButton, withTheme, Text, Icon, DataTable, TouchableRipple, Searchbar } from 'react-native-paper';
...
function DirectoryView({ path, showHeadImg, onPressItem, onLongPressItem, onError, width, height, style, onRef }) {
  ...
  return <Card style={{ width, ...style }}>
    {showHeadImg && <Card.Cover source={require('../assets/background-4.jpg')} style={{ height: 128 }} />}
    <Card.Title style={{ marginVertical: 10 }} titleVariant='headlineMedium' title={pathName == '' ? 'Drive' : pathName}></Card.Title>
    <Card.Content> 
    ...
    <View>
      <DataTable>
        ...
        {filteredList.map((item, j) => (
          <TouchableRipple key={j} onPress={() => onPressItem(item)} onLongPress={() => onLongPressItem(item)}>
            <DataTable.Row>
              <DataTable.Cell>
